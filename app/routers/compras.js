...
module.exports = async function (app) {
                ...
                if (validaRequisicao(response.statusCode, req, res)) {
                    ...
                    for (var i = 0; i < Object.keys(body.data).length; i++) {
                        const finallista = {
                            ...
                            tipo: body.data[i].tipoCompras.tipo,
                            cidade: {
    ...
    app.get('/app/' + rota + '/create', function (req, res) {
        ...
        if (!req.session.token) {
            ...
            request({
                url: process.env.API_HOST + "tipo-compras",
                method: "GET",
                json: true,
                headers: {
                    "content-type": "application/json",
            ...
            request({
                ...
                if (validaRequisicao(response.statusCode, req, res)) {
                    ...
                    res.format({
                        html: function () {
                            res.render(rota + '/Create', { itensCidades: cidades, itensTipo: compras, page: rota, informacoes: req.session.usuario });
                        }
                    });
                }
                            ...
                            res.format({
                                html: function () {
                                    res.render(rota + '/Edit', {
                                        ...
                                        tipoId: body.data.tipoCompras.id,
