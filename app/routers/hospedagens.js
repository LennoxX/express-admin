...
module.exports = async function (app) {
                ...
                if (validaRequisicao(response.statusCode, req, res)) {
                    ...
                    for (var i = 0; i < Object.keys(body.data).length; i++) {
                        const finallista = {
                            ...
                            tipo: body.data[i].tipoHospedagem.tipo,
                            cidade: {
    ...
    app.get('/app/' + rota + '/create', function (req, res) {
        ...
        if (!req.session.token) {
            ...
            request({
                url: process.env.API_HOST + "tipo-hospedagem",
                method: "GET",
                json: true,
                headers: {
                    "content-type": "application/json",
            ...
            request({
                ...
                if (validaRequisicao(response.statusCode, req, res)) {
                    ...
                    res.format({
                        html: function () {
                            res.render(rota + '/Create', { itensCidades: cidades, itensTipo: tipohospedagem, page: rota, informacoes: req.session.usuario });
                        }
                    });
                }
