...
module.exports = async function (app) {
        ...
        if (req.file.size > 220220) {
            ...
            request({
                ...
                if (response.statusCode != 200) {
                    req.flash("danger", "Item não salvo. oi " + body.errors);
                } else {
    ...
    app.post('/app/' + rota + '/list', function (req, res) {
        if (!req.session.token) {
            ...
            var url;
            if (req.body.busca) {
                url = process.env.API_HOST + "topicos/tipo/MUSEUS/" + req.body.busca + '/' + req.body.page + "/" + req.body.size;
            } else {
                url = process.env.API_HOST + "topicos/tipo/MUSEUS/" + req.body.page + "/" + req.body.size + "?sort=titulo!asc";
            }
            teste = request({
                url: encodeURI(url),
                method: "GET",
