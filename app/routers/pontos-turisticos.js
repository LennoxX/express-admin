...
module.exports = async function (app) {
    ...
    app.get('/app/' + rota + '/list', function (req, res) {
        ...
        if (!req.session.token) {
            ...
            teste = request({
                url: process.env.API_HOST + 'topicos/tipo/PONTOS_TURISTICOS/0/10?sort=titulo!asc',
                method: "GET",
                json: true,
                headers: {
    ...
    app.post('/app/' + rota + '/list', function (req, res) {
        if (!req.session.token) {
            ...
            var url;
            if (req.body.busca) {
                url = process.env.API_HOST + "topicos/tipo/PONTOS_TURISTICOS/" + req.body.busca + '/' + req.body.page + "/" + req.body.size;
            } else {
                url = process.env.API_HOST + "topicos/tipo/PONTOS_TURISTICOS/" + req.body.page + "/" + req.body.size + "?sort=titulo!asc";
            }
            teste = request({
                url: encodeURI(url),
                method: "GET",
