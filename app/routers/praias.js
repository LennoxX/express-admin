...
module.exports = async function (app) {
    ...
    app.post('/app/' + rota + '/list', function (req, res) {
        if (!req.session.token) {
            ...
            var url;
            if (req.body.busca) {
                url = process.env.API_HOST + "topicos/tipo/PRAIAS/" + req.body.busca + '/' + req.body.page + "/" + req.body.size;
            } else {
                url = process.env.API_HOST + "topicos/tipo/PRAIAS/" + req.body.page + "/" + req.body.size + "?sort=titulo!asc";
            }
            teste = request({
                url: encodeURI(url),
                method: "GET",
